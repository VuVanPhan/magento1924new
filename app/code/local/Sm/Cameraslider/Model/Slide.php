...
-------------------------------------------------------------------------*/
class Sm_Cameraslider_Model_Slide extends Mage_Core_Model_Abstract
{
    ...
    public function getOptionStatus()
    {
        $option = new Varien_Object(array(
            self::STATUS_ENABLED => Mage::helper('sm_cameraslider')->__('Enable'),
            self::STATUS_DISABLED => Mage::helper('sm_cameraslider')->__('Disable'),
        ));
    ...
    public function getLoaderOpacity()
    {
        ...
        foreach($array as $a)
        {
            $array2[] = array(
                'value' => $a,
                'label' => Mage::helper('sm_cameraslider')->__("$a")
            );
        }
    ...
    {
        $collection = Mage::getModel('sm_cameraslider/sliders')->getCollection()->addSlideFilter($this)->setOrder('priority', 'asc');
        $sliders = array();
