<?php
class Sm_Cameraslider_Adminhtml_CamerasliderController extends Mage_Adminhtml_Controller_Action{
    ...
    /*
        Init actions
        @return Sm_Cameraslider_Adminhtml_Cameraslider
    */
    public function _initAction()
    {
        // Load layout , set active menu and breadcrumbs
        $this->loadLayout()
            ->_setActiveMenu('sm/cameraslider')
            ->_addBreadcrumb(
                Mage::helper('sm_cameraslider')->__('Slide'),
                Mage::helper('sm_cameraslider')->__('Slide')
            )
            ->_addBreadcrumb(
                Mage::helper('sm_cameraslider')->__('Slide'),
    ...
    public function indexAction(){
        $this->_title($this->__('Cameraslider'))
            ->_title($this->__('Camera Slide'));
        $this->_initAction();
        $block = $this->getLayout()->createBlock('sm_cameraslider/adminhtml_cameraslider', 'cameraslider');
    ...
    {
        $this->loadLayout();
        $this->getResponse()->setBody( $this->getLayout()->createBlock( 'sm_cameraslider/adminhtml_cameraslider_grid' )->toHtml() );
    }
    ...
    public function editAction()
    {
        ...
        if(($model->getId()) || ($slideId == 0))
        {
            ...
            $this->_initAction();
            $this->_title(Mage::helper('sm_cameraslider')->__('Manager Slide'));

            if($model->getId()) {
                $this->_title($model->getData('name_slide'));
            }else{
                $this->_title(Mage::helper('sm_cameraslider')->__('Add New Slide'));
            }
//            $this->loadLayout();
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('sm_cameraslider/adminhtml_cameraslider_edit'));
            $this->_addLeft($this->getLayout()->createBlock('sm_cameraslider/adminhtml_cameraslider_edit_tabs'));
            $this->getLayout()->getBlock('head')->setCanLoadTinyMce(true);

            // 5. Render Layout
            $this->renderLayout();
        }else{
            $this->_getSession()->addError(Mage::helper('sm_cameraslider')->__('Manager Slide item does not exits'));
        ...
        if($filter)
        {
            ...
            try{
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('sm_cameraslider')->__('The slide item has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('sm_cameraslider')->__('An error occurred while saving the slide item.')
                );
            }
        }
        Mage::getSingleton( 'adminhtml/session' )->addError( Mage::helper( 'sm_cameraslider' )->__( 'No data found to save' ) );
        $this->_redirect( '*/*/' );
    }
        ...
        if(is_array($slide_id))
        {
            try{
                ...
                foreach($slide_id as $s)
                {
                    ...
                    if(!$slide->getId())
                    {
                        Mage::throwException(Mage::helper('sm_cameraslider'))->__('Unable to find a slide');
                    }
                    $slide->delete();
                }
                $this->_getSession()->addSuccess(Mage::helper('sm_cameraslider')->__('The Slide has been delete'));
                $this->_redirect( '*/*/' );
                ...
                $this->_redirectReferer();
                $this->_getSession()->addException($e,
                    Mage::helper('sm_cameraslider')->__('An error occurred while deleting the slide item.')
                );
            }
        }
    }
    ...
    {
        $this->loadLayout();
        $this->getResponse()->setBody($this->getLayout()->createBlock('sm_cameraslider/adminhtml_cameraslider_edit_tab_sliders')->toHtml() );
    }
    ...
    public function addSlidersAction()
    {
        ...
        $this->_initAction();
        $this->_title(Mage::helper('sm_cameraslider'))->__('Sliders');
        if($modelSliders->getId())
        {
            $this->_title($modelSliders->getData('sliders_title'));
        }else{
            $this->_title(Mage::helper('sm_cameraslider')->__('New Sliders'));
        }

        // Load layout and render layout
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()->createBlock('sm_cameraslider/adminhtml_sliders_edit'));
        $this->_addLeft($this->getLayout()->createBlock('sm_cameraslider/adminhtml_sliders_edit_tabs'));
        $this->getLayout()->getBlock('head')->setCanLoadTinyMce(true);
...
//        $this->_addContent( $this->getLayout()->createBlock('sm_cameraslider/adminhtml_sliders_video', 'adminhtml_sliders_video'));
        $block = $this->getLayout()->createBlock('sm_cameraslider/adminhtml_sliders_video', 'adminhtml_sliders_video');
        $this->_addContent($block);
        $this->renderLayout();
    }
    ...
    public function UploadFile($id)
    {
        ...
        if(isset($_FILES['image_url']['name']) && ($_FILES['image_url']['tmp_name'] != null))
        {
            try{
                ...
                $path = Mage::getBaseDir('media').DS.'sm'.DS.'cameraslider'.DS.'images'.DS;
                $uploader->save($path, $_FILES['image_url']['name']);

                $modelSliders = Mage::getModel('sm_cameraslider/sliders')->load($id);
                $modelSliders->setData('image_url', $_FILES['image_url']['name']);
                $modelSliders->save();
            }catch (Exception $e){
                $this->_getSession()->addException($e,
                    Mage::helper('sm_cameraslider')->__('An error occurred while upload file sliders.')
                );
            }
        }
    }
...
//            unset($sliders['slide_id']);
    ...
    public function saveSlidersAction()
    {
        ...
        if($filterData)
        {
            ...
            try{
                $modelSliders->save();
                $this->_getSession()->addSuccess(
                    Mage::helper('sm_cameraslider')->__('The sliders item has been saved.')
                );
    ...
    public function deleteSlidersAction()
    {
        ...
        if(is_numeric($sliders_id))
        {
            try{
                ...
                if(!$modelSliders->getId())
                {
                    Mage::throwException(Mage::helper('sm_cameraslider'))->__('Unable to find a sliders');
                }else{
                    $modelSliders->delete();
                }
                ...
                $this->_getSession()->addSuccess(Mage::helper('sm_cameraslider')->__('The Sliders has been delete'));
            }catch (Mage_Core_Exception $e){
                $this->_redirect('*/*/edit', array(
                    'id'        => $slide_id,
                    'activeTab' => 'form_slide'
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('sm_cameraslider')->__('An error occurred while deleting the sliders item.')
                );
            }
        }
    }
    ...
    public function ajaxSaveAction()
    {
        if($data = $this->getRequest()->getPost())
        {
            ...
            {
                case 'priority' :
                    $model = Mage::getModel('sm_cameraslider/sliders')->load($id);
                    if($model->getId())
                    {
                        $model->setData($attr, $value);
                        $model->save();
                    }else{
                        $out['message'] = Mage::helper('sm_cameraslider')->__('Sliders not avaiable');
                    }
            }
