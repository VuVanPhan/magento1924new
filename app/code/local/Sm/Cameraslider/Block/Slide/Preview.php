...
//	}
	...
	protected function _toHtml()
	{
		...
		$folder_cache = $folder_cache.'/Sm/Cameraslider/';
	...
	public function renderLayers($slider)
	{
		...
		foreach($slider->getLayers() as $layer)
		{
			...
			$classLayer = $class_layer ? "class='.$class_layer.'" : '';
			...
			if($type == 'text')
			{
				...
				$output .= "<em style='position: relative;z-index:{$order};{$visibility}{$font_size}{$float}{$left}{$right}{$top}{$bottom}width:100%;height:100%;{$min_width}{$min_height}{$bg_color}{$color}{$text_align}{$textBold}{$textItalic}{$textUnderline}{$textTransform}' {$classLayer}>$html_text</em>";
				$output .= "</div>";
			}
			elseif ($type == 'image')
			{
			...
			elseif ($type == 'video')
			{
				...
				if((isset($service_video)) && ($service_video == 'youtube'))
				{
					...
					$src_video = $httpBases."www.youtube.com/embed/{$layer->getData('video_id')}".'?'.$controls.'&'.$autoplay.'&'.$loop;
					$htmlVideo .= "<iframe class='{$video_class}' data-autoplay='{$data_play}' src='{$src_video}' width='{$video_width}' height='{$video_height}' style='z-index:{$order};{$video_left}{$video_top}'></iframe>";
